...
\subsubsection{Convolução e Filtros}
...
\begin{figure}[ht]
    ...
    \caption{O resultado de aplicação de um \kernel\ simples para detecção de bordas verticais a uma foto. O \kernel\ é aplicado a cada pixel da imagem, substituindo o valor deste pela soma de sua vizinhança ponderada pelos valores da matriz. Foto original obtida de \cite{diliffEnglishLookingEast2015}.}
    \label{fig:conv}
\end{figure}

No contexto de redes neurais convolucionais, isso consiste em neurônios cuja coleção de dados realiza uma convolução sobre os dados de entrada.
